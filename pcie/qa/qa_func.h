...
#define CARD_LOCAL_IN_CH_NUM 0
#define CARD_LOCAL_OUT_CH_NUM 8
...
extern "C"
{
    ...
    typedef struct
    {
        ...
        DACConfigReg_t m_DAC_config_reg[CARD_LOCAL_OUT_CH_NUM];
        DEConfigReg_t m_DE_config_reg[CARD_LOCAL_OUT_CH_NUM];
        SampleConfigReg_t m_sample_config_reg[CARD_LOCAL_IN_CH_NUM];
        decisionCntReg_t m_decision_cnt_reg[CARD_LOCAL_IN_CH_NUM];
        uint32_t m_device_id;
        uint32_t m_adc_mem_status;  // adc采集状态
        uint32_t m_adc_corr_status; // adc解调状态
        uint32_t m_dac_play_status; // dac播放状态
        uint64_t m_dac_ram_base[CARD_LOCAL_OUT_CH_NUM];
    } QAUserReg_t;
