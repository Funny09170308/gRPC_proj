...
static int get_awg_base_ch_by_index(int awg_count, int index)
{
    ...
    static const int awg_base_2[2] = {1, 5};
    static const int awg_base_3[3] = {9, 1, 5};
    static const int awg_base_4[4] = {9, 1, 5, 13};
...
static void build_awg_map(PcieBoardInfo *info)
{
    ...
    info->awg_ch_num = awg_index * C_LNAWG_CHANNEL_NUM;
...
static void build_qa_map(PcieBoardInfo *info)
{
    ...
    info->qa_in_ch_num = qa_index * CARD_LOCAL_IN_CH_NUM;
    info->qa_out_ch_num = qa_index * CARD_LOCAL_OUT_CH_NUM;

    for (i = 0; i < qa_index; i++)
    {
        int chip = qa_chip_list[i];
        int base_in = i * CARD_LOCAL_IN_CH_NUM + 1;
        int base_out = i * CARD_LOCAL_OUT_CH_NUM + 1;

        /* IN */
        for (uint8_t j = 0; j < CARD_LOCAL_IN_CH_NUM; ++j)
        {
            info->qa_in_map[base_in + j].chip_id = chip;
            info->qa_in_map[base_in + j].local_ch = j + 1;
        }

        /* OUT */
        for (uint8_t j = 0; j < CARD_LOCAL_OUT_CH_NUM; ++j)
        {
            info->qa_out_map[base_out + j].chip_id = chip;
            info->qa_out_map[base_out + j].local_ch = j + 1;
        }
    }
}
...
int pcie_dev_init(void)
{
    ...
    for (i = 0; i < (uint32_t)g_pcie_board_info.board_num; i++)
    {
        ...
        else if (dev_type == DEV_TYPE_AWG_4CH)
        {
            g_pcie_board_info.items[i].dev_type = DEV_TYPE_AWG_4CH;
            P_LOG_INITIAL("Card %d type: %#x", i, g_pcie_board_info.items[i].dev_type);
        }
